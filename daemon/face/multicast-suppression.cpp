...
getSeed()
{
  ...
  getline(f, line); // reads a line from the file and stores it in the line string variable.
  NFD_LOG_INFO ("getline " << line);
  auto seed = std::stoi (line); //to convert the string line to an integer value
...
int
getRandomNumber_saurab(int upperBound)
{
  NFD_LOG_INFO ("get random number called ");
  if (!seeded)
  {
...
NameTree::getSuppressionTimer(const std::string& name)
{
  ...
  NFD_LOG_INFO("val" << val);
  suppressionTime = (val == UNSET) ? minSuppressionTime : val;
  NFD_LOG_INFO("suppressionTime" << suppressionTime);
  // if (suppressionTime == 0) {
    // NDN_LOG_INFO("suppression time is zero");
    // return time::milliseconds (0);
  // }
  NFD_LOG_INFO("static_cast<int> (2*suppressionTime)" << static_cast<int> (2*suppressionTime));
  NFD_LOG_INFO("get random number from 12 " << getRandomNumber(12));
  // time::milliseconds suppressionTimer (getRandomNumber(static_cast<int> (2*suppressionTime))); // timer is randomized value
  time::milliseconds suppressionTimer = time::milliseconds(getRandomNumber(static_cast<int>(2 * suppressionTime)));
...
MulticastSuppression::getDelayTimer(Name name, char type)
{
  ...
  auto nameTree = getNameTree(type);
  NFD_LOG_INFO("nameTree : " << nameTree);
    NFD_LOG_INFO("name.getprefix(-1).toURIfgjldkfgj");

  NFD_LOG_INFO("name.getprefix(-1).toURI" << name.getPrefix(-1).toUri()); ///uofm/ahdhg/%00
    NFD_LOG_INFO("name.getprefix(-1).toURI");
